...
const Settings: React.FC<SettingsProps> = ({ settings, onUpdate }) => {
  ...
  const wipeAllData = async () => {
    ...
    try {
      ...
      alert("System Wiped Successfully. The application will now reload.");
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in duration-500 pb-20">
      <header>
        ...
        <p className="text-slate-500 font-medium">Manage business identity, tax compliance, and data lifecycle.</p>
      </header>
      ...
      <form onSubmit={handleSave} className="space-y-10">
        <div className="bg-white p-8 md:p-14 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-12 relative overflow-hidden">
        ...
        <div className="flex justify-end items-center gap-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-14 py-5 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-2xl shadow-slate-200 hover:bg-black transition-all flex items-center gap-3 active:scale-[0.98] disabled:opacity-50"
          >
            {loading ? (
      ...
      <div className="mt-20 border-t border-slate-200 pt-16">
        <div className="bg-rose-50 border border-rose-100 p-10 rounded-[3rem] space-y-6">
           ...
           <p className="text-sm font-medium text-rose-800 max-w-2xl">
             Wiping all data will reset the system to a clean state. This deletes all transaction history, expenses, supplier ledgers, and products. This is recommended before final client handover or for a fresh start.
           </p>
           <button 
             onClick={wipeAllData}
             disabled={isWiping}
